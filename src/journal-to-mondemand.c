...
//    unsigned short size = (unsigned short) ( ( ( header[0] << 8) & (255 << 8))
//                                           | ( ( header[1] << 0) & (255)));
    ...
    lwes_event_from_bytes (event, buf, size, 0, dtmp);
    {
      ...
      for (LWES_U_INT_16 i = 0; i < num; i++)
        {
          ...
          LWES_LONG_STRING tmp_key;
          LWES_INT_64 tmp_value;
          ...
          lwes_event_get_INT_64 (event, key_buffer, &tmp_value);
          if (tmp_type == NULL)
            {
